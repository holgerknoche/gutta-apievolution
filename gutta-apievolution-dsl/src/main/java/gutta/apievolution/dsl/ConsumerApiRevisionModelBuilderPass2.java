 ...
 */
class ConsumerApiRevisionModelBuilderPass2 extends
        ApiRevisionModelBuilderPass2<ConsumerApiDefinition, ConsumerRecordType, ConsumerField, ConsumerEnumType, ConsumerEnumMember, ConsumerOperation> {

    public ConsumerApiRevisionModelBuilderPass2(String sourceName) {
        super(sourceName);
    }

    public void augmentConsumerRevision(final ApiRevisionParser.ApiDefinitionContext apiRevisionSpec, ConsumerApiDefinition apiDefinition) {
        this.augmentRevision(apiRevisionSpec, apiDefinition, Optional.empty());
    }

    @Override
    protected ConsumerField createField(
            final ApiRevisionParser.FieldContext context, final String name, final String internalName, final Type type,
    ...
    @Override
    protected ConsumerEnumMember createEnumMember(final ApiRevisionParser.EnumMemberContext context, final String name, final String internalName,
            final ConsumerEnumType owner) {
        return owner.newEnumMember(name, internalName);
    }

    @Override
    protected ConsumerOperation createOperation(final ApiRevisionParser.OperationContext context, final String name, final String internalName,
