...
class ProviderApiRevisionModelBuilderPass1
        extends ApiRevisionModelBuilderPass1<ProviderApiDefinition, ProviderRecordType, ProviderField, ProviderEnumType, ProviderEnumMember, ProviderOperation>
    ...
    public ProviderApiRevisionModelBuilderPass1(String sourceName) {
        super(sourceName);
    }
     ...
     */
    public ProviderApiDefinition buildProviderRevision(final int revision, final ApiRevisionParser.ApiDefinitionContext apiRevisionSpec,
    ...
    @Override
    protected ProviderApiDefinition createRevision(final ApiRevisionParser.ApiDefinitionContext context, final QualifiedName name,
            final Set<Annotation> annotations, final ProviderApiDefinition predecessor) {
        return new ProviderApiDefinition(name, annotations, this.revision, predecessor);
    }

    @Override
    protected ProviderRecordType createRecordType(final ApiRevisionParser.RecordTypeContext context, final String name, final String internalName,
            final int typeId, final ProviderApiDefinition currentRevision, final Abstract abstractFlag, RecordKind recordKind) {
        ...
        predecessor.ifPresent(record -> {
            if (record.getRecordKind() != recordKind) {
                throw new APIResolutionException(context.refToken, "The predecessor of an exception may not be a record (or vice versa).");
            }
        });

        return currentRevision.newRecordOrExceptionType(name, internalName, typeId, abstractFlag, recordKind, noSuperTypes(),
                predecessor.orElse(noPredecessor()));
    }

    private Optional<ProviderRecordType> resolvePredecessorRecord(final String name, final boolean explicitReference, final Token refToken,
            final Token nameToken) {
        return this.resolvePredecessorUDT(name, ProviderRecordType.class, explicitReference, refToken, nameToken, "a record type");
    }

    @Override
    protected ProviderEnumType createEnumType(final ApiRevisionParser.EnumTypeContext context, final String name, final String internalName, final int typeId,
    ...
    private <U extends UserDefinedType<?>> Optional<U> resolvePredecessorUDT(final String name, final Class<U> udtClass, final boolean explicitReference,
            final Token refToken, final Token nameToken, final String typeErrorMessage) {
        ...
        if (explicitReference) {
            ...
            predecessorRevision = this.previousRevision
                    .orElseThrow(() -> new APIResolutionException(refToken, "Replaces clause specified, but no predecessor model is available."));
        } else {
