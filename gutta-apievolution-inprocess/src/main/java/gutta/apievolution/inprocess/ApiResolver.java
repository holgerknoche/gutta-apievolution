...
public class ApiResolver {	
	...
	private <T> T createApi(ConsumerApiDefinition consumerApiDefinition, Class<T> apiType) {
		// Locate API provider for the provider API object and attempt to retrieve the API object for the
		// requested version
	...
	private Object createProviderApi(String providerApiName, int providerApiRevision) {
		...
		if (providerApi == null) {
			throw new NoProviderApiException("Provider API provider for API '" + providerApiName + "' returned no valid API object for revision " + providerApiRevision + ".");
		}
